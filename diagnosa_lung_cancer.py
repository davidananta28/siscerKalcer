...
#BENAR/SALAH
df['Benar/Salah'] = np.where(
    ((df['Hasil_Diagnosa'] == "TERKENA LUNG CANCER") & (df['LUNG_CANCER'] == 1)) |
    ((df['Hasil_Diagnosa'] == "TIDAK TERKENA") & (df['LUNG_CANCER'] == 0)),
    'Benar',
    'Salah'
)

#TAMPILKAN HASIL 10 DATA AWAL
df_run = df.head(10).copy()
cols_show = ['AGE','skor_gejala','skor_risiko','skor_gejala_norm','skor_risiko_norm',
             'Fuzzy_Score','Hasil_Diagnosa','LUNG_CANCER','Benar/Salah']
print("\n=== HASIL DIAGNOSA & BENAR/SALAH (10 DATA AWAL) ===")
print(df_run[cols_show].to_string(index=False))

#RINGKASAN BENAR/SALAH & AKURASI
summary = df['Benar/Salah'].value_counts()
print("\n=== RINGKASAN BENAR/SALAH SELURUH DATA ===")
for k, v in summary.items():
    print(f"{k}: {v}")

akurasi = (df['Benar/Salah'] == 'Benar').mean() * 100
print(f"\nAkurasi Prediksi (seluruh data): {akurasi:.2f}%")